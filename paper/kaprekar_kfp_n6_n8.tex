...
confined to $b\le 10$. The existence halves of these classifications
refine infinite families constructed by Jordan in 1964; the
completeness halves are, to our knowledge, new. The proofs are
computer-assisted but exact: a
...
replaced at $n=8$ by a parity-blind $3\mid b$ family, which provably
recurs at $n=10$; the general-$n$ law behind these divisors remains
open.
\end{abstract}
...
non-repdigit $n$-digit integer. Hasse and Prichett \cite{HP} determined
...
$r$-digit $g$-adic integers, a line of work going back to Jordan
\cite{Jordan64}, whose base-ten corollary prefigures the decadic
classification of \cite{PLL}. Thakur \cite{Thakur19} settled the odd
...
for $b=2$ and $3\mid b$ (the $3\mid b$ direction being consistent with
Jordan's odd-length construction), whereas five-digit Kaprekar
\emph{constants} are far rarer.

The existence side of the second axis goes back further than we knew
when this project began. In 1964, Jordan \cite{Jordan64} constructed,
for every even digit length, infinite arithmetic progressions of bases
carrying a fixed point.\footnote{Jordan works with multisets of digits,
permitting leading zeros; we classify genuine $n$-digit integers
(leading digit nonzero). The difference matters only in marginal
small-base cases and affects no statement in this paper.} In our
notation his families include the progression $b\equiv 10\pmod{15}$
for every even $n$ (specialising at $n=4$ to the Hasse--Prichett
family), progressions supported on $b\equiv 13\pmod{21}$ and on
$b\equiv 28\pmod{51}$, a six-digit example at $b=13$, and a base
carrying two distinct six-digit fixed points at $b=55$; he closed by
remarking that ``a general formula would be desirable''. What has
remained open since 1964 is the \emph{completeness} question: whether
such constructions exhaust the admissible bases. This paper settles
that question for the first two even lengths, proving the full
characterization (both directions) for $n=6$ and $n=8$.
...
Table~\ref{tab:n6}, apart from four exceptional points confined to
bases $b\in\{2,4,6\}$ (each such base already being covered by the
...
\]
that is, $A_\sigma\,d = B\,c_\sigma + e_\sigma$ with integer data. Here
$A_\sigma$ is the $h\times m$ integer matrix whose $i$-th row lists the
coefficients of the unknowns $(d_0,\dots,d_{m-1})$ in the left-hand
expression $E_{\sigma(i)}-E_{\sigma(n-1-i)}-d_i$, while $c_\sigma$ and
$e_\sigma$ in $\mathbb{Z}^h$ collect, row by row, the coefficient of $B$
and the constant term on the right. Every fixed point lies in at least
one cell (ties are broken arbitrarily; all cell inequalities are
closed), so classifying all cells classifies all fixed points.

Each cell is classified over $\mathbb{Q}$ by row reduction in exact
rational arithmetic. Writing $\Delta:=\det A_\sigma$ when the system is
square and of full rank, a unique solution then has the closed form
$d_i=(\alpha_i B+\beta_i)/\Delta$. The cells fall into four classes:
(a) a unique solution valid for all $B$; (b) a unique solution only at a
single value of $B$; (c) inconsistent (no solution); and (d)
underdetermined (a positive-dimensional solution set). For cells in
class (a) we then impose the full validity system --- the sorting
...
\smallskip
\noindent\textbf{Worked examples ($n=6$).} To make the four classes
concrete, take $n=6$, where the six successor expressions are
$E=(d_0,\,d_1,\,d_2-1,\,B-d_2,\,B-d_1,\,b-d_0)$ in channel $t=0$ (three
unknowns) and $E=(d_0,\,d_1-1,\,B,\,B,\,B-d_1,\,b-d_0)$ in channel $t=1$
(two unknowns). One representative of each class:

\emph{(a) Unique for all $B$ --- an infinite family.} In channel $t=0$,
the ordering $[\,B-d_2,\,d_0,\,d_1,\,B-d_1,\,b-d_0,\,d_2-1\,]$ yields
\[
-d_0-2d_2=-B-1,\qquad 2d_0-d_1=B+1,\qquad 2d_1-d_2=B,
\]
with the unique solution
$d=\bigl(\tfrac{7B+5}{9},\,\tfrac{5B+1}{9},\,\tfrac{B+2}{9}\bigr)$,
integral and valid for all $b\ge 8$. This is precisely the
$b\equiv 8\pmod 9$ family of Table~\ref{tab:n6}, with $\Delta=9$.

\emph{(b) Unique at a single $B$.} In channel $t=1$, the ordering
$[\,d_0,\,B,\,B,\,b-d_0,\,d_1-1,\,B-d_1\,]$ yields $d_1=B$, $2d_1=B+1$,
and $d_0=1$; the first two force $B=1$, so this cell contributes a fixed
point only at the single base $b=2$.

\emph{(c) Inconsistent.} In channel $t=0$, the ordering
$[\,d_0,\,d_2-1,\,d_1,\,b-d_0,\,B-d_1,\,B-d_2\,]$ yields both $d_2=B$ and
$d_2=B+1$; row reduction leaves the residual equation $0=-1$, so the
cell is empty for every base.

\emph{(d) Underdetermined.} In channel $t=0$, the ordering
$[\,d_0,\,d_2-1,\,d_1,\,B-d_2,\,b-d_0,\,B-d_1\,]$ yields $d_1=B$ (twice)
together with $d_0-d_1+d_2=B+2$; the coefficient matrix has rank $2<3$,
leaving one free parameter. Such cells are closed by the linear program
of principle (ii) below.
...
therefore cost efficiency, never correctness.\footnote{One documented
instance: in channel $t=3$ the leading-digit constraint was implemented
as $d_0\ge1$ where the exact condition is $d_0\ge2$. The slack is in
the safe direction, and that channel is empty in any case.}
...
\begin{table}[ht]
\caption{The four families for $n=6$ ($B=b-1$; missing $d$-components
are $0$). Origins: the $b\equiv10\ (15)$ family is Jordan's
\cite{Jordan64}; the $b\equiv6\ (7)$ family contains Jordan's examples
at $b=13,55$ and his progression $b\equiv13\ (21)$ as a one-third
density subprogression; the $b\equiv8\ (9)$ and $2\mid b$ families
appear to be new.}\label{tab:n6}
\begin{tabular}{clll}
\toprule
...
The four exceptional points have difference vectors $(1,1,1)$ at
$b=2$; $(3,1,1)$ and $(3,3,1)$ at $b=4$; and $(5,3,1)$ at $b=6$.
...
\begin{table}[ht]
\caption{The six families for $n=8$. Origins: the $b\equiv10\ (15)$
family and the progression $b\equiv13\ (21)$ appear in Jordan
\cite{Jordan64}; our $b\equiv11\ (17)$ family contains his
$b\equiv28\ (51)$ progression as a one-third density subprogression;
the $b\equiv12\ (15)$, $b\equiv14\ (17)$ and $3\mid b$ families appear
to be new.}\label{tab:n8}
\begin{tabular}{clll}
\toprule
Channel & Congruence & $d$-vector & First members \\
\midrule
...
after deduplication: cells sharing an identical solution merge, the
$b\equiv 10\ (15)$ family arising in four distinct cells through the
tied components $B/3$ of its $d$-vector, the other four families in one
cell each, and one live cell being eliminated by integrality; $505$
bounded windows, all exhausted --- $15$ window hits in total, none
beyond $b=14$; $5152$
...
for every base. In the extension range $61\le b\le 100$ the scan finds
fixed points in $22$ of the $40$ bases, $26$ points in all, each one a
member of the predicted families.
...
family), and $t\ge2$ is overdetermined (finite collapse). Separately,
the divisor of the single $t=1$ family is even only at $n=6$: it is
$2$ there, forcing $2\mid b$ and creating the even/odd asymmetry, but
$3$ at $n=8$ (and again at $n=10$; see Remark~\ref{rem:n10}), which is
parity-blind. Since all $t=0$ moduli are odd, the parity wall of $n=6$
appears special to that divisor being $2$ rather than a general
feature; what fixes the $t=1$ divisor for general $n$ we leave open.

\begin{remark}[$n=10$]\label{rem:n10}
The pattern continues at $n=10$, and it corrects a tempting guess.
Substituting
\[
d=\Bigl(\tfrac{2B+2}{3},\,\tfrac{2B+2}{3},\,\tfrac{B+1}{3},\,
\tfrac{B+1}{3},\,0\Bigr)
\]
into the successor string of Lemma~\ref{lem:succ} and re-sorting
reproduces $d$ for every $B\equiv 2\pmod 3$, $B\ge 2$ (all digit and
ordering constraints hold; in the sorted string the tie
$d_{m-1}-1=B-d_0$ closes the check). Hence \emph{every} base
$b\equiv 0\pmod 3$ admits a ten-digit fixed point: the distinguished
$t=1$ family has divisor $3$, \emph{not} the value $h-1=4$ that the
$n=6,8$ divisors $(2,3)$ might suggest, and no even-base advantage
returns; $3\mid b$ persists, parity-blind, as at $n=8$. A ground-truth
scan of all bases $b\le 45$ (ancillary file \texttt{check\_n10.py})
supports the rest of the qualitative picture: in that range the
channels $t\ge 2$ contribute nothing beyond $b=2$, and the $t=0$
families visibly extend those of Table~\ref{tab:n8} --- for example
$b\equiv 10\pmod{15}$ recurs (members $b=10,25,40$) with
$d=\bigl(\tfrac{3B+3}{5},\tfrac{B}{3},\tfrac{B}{3},\tfrac{B}{3},
\tfrac{B+1}{5}\bigr)$, the $n=8$ family of the same congruence with
one additional $\tfrac{B}{3}$ entry. For the $b\equiv10\pmod{15}$
family this cross-length uniformity is not new: it is precisely the
parametrisation of Jordan's 1964 construction \cite{Jordan64}, which
our verification confirms independently. Whether these are \emph{all}
the infinite families at $n=10$ awaits the full cell classification.
\end{remark}

Two problems remain open. A complete $n=10$ classification is feasible
once the $10!$ cell enumeration is reduced to the riffle-shuffle
interleavings of the two internally-ordered successor blocks (a few
hundred per channel, versus $10!$ naively). And the general-$n$ law
governing the $t=1$ divisor is genuinely open: its first three values
$2,3,3$ do not follow the naive $h-1$, a reminder that two data
...
\bibitem{Jordan64}
J.~H. Jordan, \emph{Self-producing sequences of digits}, Amer. Math.
Monthly \textbf{71} (1964), 61--64.

\bibitem{HP}
H.~Hasse and G.~D. Prichett, \emph{The determination of all four-digit
